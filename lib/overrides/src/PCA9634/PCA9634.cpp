...
//  CONSTRUCTOR
//
PCA9634::PCA9634(const uint8_t deviceAddress, SoftWire *wire) {
...
uint8_t PCA9634::getAddress() { return _address; }
...
uint8_t PCA9634::channelCount() { return _channelCount; }
...
uint8_t PCA9634::getMode1() { return readRegister(PCA963X_MODE1); }

uint8_t PCA9634::getMode2() { return readRegister(PCA963X_MODE2); }

/////////////////////////////////////////////////////
//
//  GROUP REGISTERS
//
uint8_t PCA9634::setGroupPWM(uint8_t value) { return writeRegister(PCA963X_GRPPWM, value); }

uint8_t PCA9634::getGroupPWM() { return readRegister(PCA963X_GRPPWM); }

uint8_t PCA9634::setGroupFREQ(uint8_t value) { return writeRegister(PCA963X_GRPFREQ, value); }

uint8_t PCA9634::getGroupFREQ() { return readRegister(PCA963X_GRPFREQ); }
...
uint8_t PCA9634::write1(uint8_t channel, uint8_t value) { return writeN(channel, &value, 1); }
...
uint8_t PCA9634::writeAll(uint8_t *arr) { return writeN(0, arr, 8); }
...
bool PCA9634::enableSubCall(uint8_t nr) {
    ...
    uint8_t mask = prev;
    if (nr == 1)
        mask |= PCA963X_MODE1_SUB1;
    else if (nr == 2)
        mask |= PCA963X_MODE1_SUB2;
    else
        mask |= PCA963X_MODE1_SUB3;
...
bool PCA9634::isEnabledSubCall(uint8_t nr) {
    ...
    if (nr == 1)
        return (mask & PCA963X_MODE1_SUB1) > 0;
    if (nr == 2)
        return (mask & PCA963X_MODE1_SUB2) > 0;
...
//  OBSOLETE
//
uint8_t PCA9634::setLedDriverMode(uint8_t mode) { return setLedDriverModeAll(mode); }
