...
// Atomic function to set (and register) an LVGL local style-property for a given part+state (selector) of an object (widget) -
// can be used in many ways due to being atomic
...
lv_style_value_t _ui_style_value_convert(lv_style_prop_t property, ui_style_variable_t value) {
    ...
    if (property == LV_STYLE_BG_COLOR || property == LV_STYLE_BG_GRAD_COLOR || property == LV_STYLE_BG_IMG_RECOLOR ||
    ...
    } else if (property == LV_STYLE_BG_GRAD || property == LV_STYLE_BG_IMG_SRC || property == LV_STYLE_ARC_IMG_SRC ||
               property == LV_STYLE_TEXT_FONT || property == LV_STYLE_COLOR_FILTER_DSC || property == LV_STYLE_ANIM ||
...
_ui_local_style_t *_ui_local_style_create(const ui_style_variable_t *style_variable_p, bool is_themeable) {
    ...
    _ui_local_styles =
        (_ui_local_style_t *)lv_mem_realloc(_ui_local_styles, (_ui_local_style_count + 1) * sizeof(_ui_local_style_t));
...
_ui_local_style_property_setting_t *_ui_local_style_property_setting_create(_ui_local_style_t *local_style_p, lv_obj_t *object_p,
    ...
    if (empty_style_property_setting_p == NULL) { // allocate new
        empty_style_property_setting_p =
            (_ui_local_style_property_setting_t *)lv_mem_alloc(sizeof(_ui_local_style_property_setting_t));
